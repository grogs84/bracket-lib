...
__export(index_exports, {
  ...
  USER_BRACKET_RAW: () => USER_BRACKET_RAW,
  buildLayout: () => buildLayout,
  toBracketInput: () => toBracketInput
});
module.exports = __toCommonJS(index_exports);

// src/samples/user-fixture.ts
function toBracketInput(user, opts = { championshipOnly: true }) {
  const useChampOnly = opts.championshipOnly !== false;
  const champRounds = /* @__PURE__ */ new Set([
    "champ 64",
    "champ 32",
    "champ 16",
    "champ 8",
  ...
  const filtered = useChampOnly ? user.matches.filter((m) => champRounds.has(m.round)) : user.matches;
  const roundMap = (r) => {
    if (!r) return null;
    if (r === "1st") return "F";
  ...
  const toRef = (p) => p ? { name: p.name, seed: p.seed, school: p.school } : void 0;
...
var USER_BRACKET = toBracketInput(USER_BRACKET_RAW, { championshipOnly: true });
